...
本目录只有一个产品中心：建设可装配的软件通用能力底座，让新软件在统一管理后台选择目标端、完整能力包和一套兼容 UI 模板后，直接获得可运行的前台框架、所选公共页面、统一后台管理内容、真实后端、SDK、配置、源码、测试和说明。平台不生成通用业务目录或虚构核心内容；开发人员只继续开发该软件独有的业务首页、目录、工作台和功能。统一管理后台是共享交付面，不是产品中心。主计划中的第一、第二、第三款软件都是装配验收对象，只验证平台能否正确创建、隔离和交付所选公共能力，不代表平台继续开发这些软件的正文业务。
...
| 任务 | 继续读取 |
|---|---|
| 框架对齐背景（无执行权） | `framework-realignment-plan.md`、ADR-0001、ADR-0010；当前工作编号和顺序只看 `end-to-end-development-plan.md` |

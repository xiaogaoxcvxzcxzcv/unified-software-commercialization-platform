...
本目录只有一个产品中心：建设可装配的软件通用能力底座，让新软件在统一管理后台选择目标端、完整能力包和一套兼容 UI 模板后，直接获得可运行的前台框架、所选公共页面、统一后台管理内容、真实后端、SDK、配置、源码、测试和说明。平台不生成通用业务目录或虚构核心内容；开发人员只继续开发该软件独有的业务首页、目录、工作台和功能。统一管理后台是共享交付面，不是产品中心。

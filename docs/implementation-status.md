...
| 产品蓝图与 Assembly | G2C-01 verified | G1-04 至 G1-11 已验证；创建/恢复、生命周期 API、可信 Extension Catalog、确定性快照/计划、本地 Full 和托管 push/PR 检查均通过；G2C-01 已把 account + entitlement + standard-a + generator + SDK + 样板 extension 的 experimental 组合闭包锁定，并通过本地 Full `-RequirePostgres` 22/22 与 PR #14 required checks | G2C-02 真实创建装配验收软件 A |
...
| Go 后端 | G1 + G2A + G2B-03 verified | Assembly 发布闭包，以及 Identity 最终用户、认证/账号 API、外部身份、注册/恢复安全投递和 Product User Access 已通过真实 PostgreSQL、本地 Full 与托管 push/PR CI；HostedInteraction 登录/账号后端、Identity proof/grant 会话绑定和真实运行时 HTTP + PostgreSQL 流已验证；Entitlement `000026`、Domain/Application/PostgreSQL Adapter、HTTP API、Outbox/Audit、幂等、复杂策略、source tuple revoke、管理查询 API 与 Access Control `entitlement.read/manage/revoke` 目录均已通过真实 PostgreSQL Full 与托管 required check | 后续 Entitlement 用户前台、SDK、完整能力包和生产部署验证 |
...
| 模块 | 当前状态 | 说明 |
|---|---|---|
| assembly | G2C-01 verified | 后端、Generator 文件安全、模板、工具/扩展目录基础、创建、工作区、durable Run/恢复和 lifecycle API 已验证；普通/实验 scope、权限、摘要、快照、Product 与环境绑定失败关闭；G2C-01 experimental 工具、SDK、样板 Extension 与 account/entitlement/standard-a 兼容闭包已通过本地 Full 22/22 和 PR #14 required checks | G2C-02 安装扩展并真实装配样板 A |
...
| entitlement | G2B-05 verified；package experimental verified candidate | G2B-01 已补齐 Feature/Policy/Validity/Grant/Revision/Ledger/Check Decision、叠加/撤销/到期语义、唯一约束、幂等和并发策略；G2B-02 已完成 `000026`、公开应用服务、PostgreSQL Adapter、HTTP API、Outbox/Audit、同 source/effect 与幂等去重、expected revision、source tuple revoke、互斥/优先级/replace/reject conflict 和真实 PostgreSQL/Full/CI 验证；G2B-03 管理查询 API、真实管理 API Client、Entitlement 管理 Blocks 和浏览器主路径已 verified；G2B-04 用户前台、SDK、Hosted account 集成、Generated Source、能力禁用边界、到期空态和专用真实浏览器 E2E 已 verified；G2B-05 包内九面、experimental Manifest、ordinary 隔离、`entitlement.summary` 运行时 ready、ST-005/006/039、本地 Full `-RequirePostgres` 22/22 和 PR #14 required check 已通过 | 仍缺 G2C 装配回归；非 ordinary available |
...
- G2C-01：`verified`。已发布并验证 experimental 组合闭包：`package.account` 1.0.0、`package.entitlement` 1.0.0、`standard-a` 0.1.0、`platform.generator` 1.0.0、`platform.sdk` 1.0.0 和 `extension.editor-tools` 1.0.0；真实机器目录测试覆盖依赖闭包、ordinary 不可见、受信工具/扩展锁定，Planner 测试覆盖真实组合的确定性 Assembly Plan。本地 Core 6/6 与 Full `-RequirePostgres` 22/22 已通过，Full 报告为 `artifacts/reviews/G2C-01/quality-gate-full-postgres.json`。代码 checkpoint `8509c3e76e46588430a017a3045da18e757baad1` 与状态同步提交 `6cc9909` 已通过 PR #14 push/pull_request required checks。下一唯一关口为 G2C-02；当前仍没有 ordinary available 包。证据：`artifacts/reviews/G2C-01/candidate-catalog-compatibility.md`。
...
- G2B-04：`verified`。`entitlement.summary` 用户前台 Block、Entitlement TypeScript SDK、Hosted account 可选权益投影、Generated Source、包内容树测试、能力禁用失败关闭边界和 OpenAPI 投影已完成；补齐 Hosted account 对空 features + 过期 `valid_until` 的到期空态映射，避免通用账号空态覆盖“权益已到期”。专用真实浏览器 E2E 在 Hosted Web 中打开 4 个 `hosted.account` interaction，覆盖有权益、无权益、到期和能力禁用；有权益页显示 `权益摘要`、`pro`、`priority_queue`，且不展示价格、支付状态或金额字段，禁用时不显示“当前权益”入口。本地 Full `-RequirePostgres` 22/22 通过，报告为 `artifacts/reviews/G2B-04/quality-gate-full-postgres-browser-e2e.json`；Hosted Web 57/57、Client UI 129/129、SDK 43/43、Admin 165/165 和生产构建均通过。G2B-05 已完成；`package.entitlement` 仍不是 ordinary available。证据：`artifacts/reviews/G2B-04/entitlement-user-sdk-source.md`。

- G2B-03：`verified`。Entitlement 管理查询契约、OpenAPI、后端 `ListCurrentEntitlements`、真实管理 API Client、`/products/:productId/entitlements` 路由和页面已完成，替换原演示 Client；页面支持服务端范围查询、授予、延长、撤销、Ledger 历史、幂等键、审计编号、撤销二次确认和 reauth 后返回当前页。浏览器验收发现并补齐 Access Control 中缺失的 `entitlement.read` 与 `entitlement.revoke` 权限目录。本地 `go test ./internal/modules/accesscontrol ./internal/modules/entitlement/... ./cmd/server`、OpenAPI 123/130、Admin 165/165、Admin build、Core 6/6、Full `-RequirePostgres` 22/22 和真实浏览器授予/流水/延长/撤销主路径已通过；提交 `8f83486` 已 push，PR #14 push run `29985749580` 与 pull_request run `29985752129` 的 `quality-gate` 和 `windows-tls` 均成功。`entitlement.table`、`entitlement.grant-panel`、`entitlement.history` 已标 ready；`package.entitlement` 仍不是 available，下一唯一关口为 G2B-04 用户前台、SDK 和源码。证据：`artifacts/reviews/G2B-03/entitlement-admin-blocks.md`。
...
- G2B-01：`verified`。Entitlement G2B-01 模型、Contract Manifest、配置 Schema、状态表、唯一约束、叠加/撤销/到期语义、幂等和并发策略已封口；新增机器契约测试校验 Manifest schema、content/manifest hash、Account 依赖、`000026_entitlement` 迁移边界和合同文本关键约束。本地 `go test -count=1 ./internal/modules/assembly/machinecontract ./internal/modules/assembly/machinecatalog`、Core 6/6 通过；提交 `01e425d` 已 push；PR #14 push run `29936790987` 和 pull_request run `29936794373` 的 `quality-gate` 与 `windows-tls` 均成功。`package.entitlement` 仍只是 contracted，不是 ordinary available。证据：`artifacts/reviews/G2B-01/entitlement-model-contract.md`。后续 G2B-02 已验证，当前唯一关口为 G2B-03。

- G2A-08：`verified`。Account 包内九面验证、experimental verified candidate 发布、ordinary 目录不可见、MachineCatalog 隔离、Product User Access 双 Product/双 Tenant 真实 PostgreSQL 专项和 formal hosted bootstrap compatibility 修复均已完成；本地 Full `-RequirePostgres` 22/22、Core 6/6、受影响 formal server 测试包通过。最终提交 `8e8dd7e` 已 push；PR #14 最新 push run `29932687031` 与 pull_request run `29932690239` 的 `quality-gate` 和 `windows-tls` 均通过。`package.account` 仍不是 ordinary available，G2C 装配回归前普通 `/create` 不可选。后续 G2B-01 已验证，当前唯一关口为 G2B-02。证据：`artifacts/reviews/G2A-08/account-package-nine-face-verification.md`。

- G2A-07：`verified`。Account SDK 22 个公开方法、`AccountSessionVault` 恢复/终态清理、配置 Schema、Manifest 内容锁、六个 Generated Source 输出、包级样板和接入文档已验证；SDK 37 项测试、生成样板 typecheck/Vitest/build、Hosted 自动测试 54/54、本地真实 PostgreSQL Full `-RequirePostgres` 22/22 通过。提交 `5b49f6d10225402b3ae448b042e2b2f060a6ead6` 的 PR #14 required check 成功：run `29921724277` 的 `windows-tls` job `88928620012` 与 `quality-gate` job `88929088864` 均通过；同一 head 的 push run `29921718889` 也通过。证据：`artifacts/reviews/G2A-07/account-sdk-generated-source.md` 与本地 Full 报告。后续 G2A-08 已验证；当前唯一严格关口为 G2B-02，且没有 available 包。

...
| identity | admin auth + G2A-04 external identity verified；G2A-04.1 hosted binding implemented | 管理认证已补救复验；最终用户认证/账号、refresh 重放撤销、外部身份 start/callback/微信一次性交换、绑定/查询/解绑、注册验证与恢复投递已通过真实 PostgreSQL、本地 Full 与托管 CI；Hosted proof/grant 的环境、会话绑定、一次性兑换和真实运行时登录/账号链已实现并通过本地 Full | G2A-04.1 托管 CI 与证据封口；生产 OIDC/微信 Provider E2E；G2A-05/G2A-06 管理/用户 UI |
| hosted-interaction | G2A-04.1 implemented, pending hosted CI/evidence | ADR-0018 所有权、`000018` 至 `000022`、interaction/browser session/completion grant、精确 return target、state/nonce/PKCE、一次性 code、CSRF、租约接管、恢复与取消/过期已实现；commit `8421568` 本地真实 PostgreSQL Full 18/18 通过 | 托管 push/PR `quality-gate` 和正式证据封口；G2A-06 Hosted UI、G2A-07 SDK/配置/源码、G2A-08 包内九面验证 |

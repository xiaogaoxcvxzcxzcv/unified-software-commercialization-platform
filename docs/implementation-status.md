...
| 完整能力包目录 | package.account verified candidate / package.entitlement contracted | Account 独立 Manifest、配置 Schema、九面合同、Product User Access 边界和目录防泄漏已验证；G2A-03 至 G2A-08 的认证账号 API、准入、外部身份、安全通知、HostedInteraction、管理 Blocks、用户 Blocks、SDK/配置/Generated Source、包内九面验证和 experimental candidate 发布均已通过；Entitlement G2B-01 模型、Manifest、唯一约束和并发规则已封口；ordinary 目录仍为空 | 下一唯一关口为 G2B-02 Entitlement 后端与迁移；仍缺 G2B-02 至 G2B-05 和 G2C 装配回归，当前没有 available 完整能力包 |

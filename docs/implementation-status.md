...
| Go 后端 | G1 verified + G2A-03 local verified candidate | Assembly 发布闭包，以及 Identity 最终用户、13 条认证/账号路由、Product User Access 管理/准入/范围撤销已通过本地真实 PostgreSQL 与 Full；G2A-03 托管 CI 待上传 | G2A-03 托管门禁、后续 Account/Entitlement 完整能力包和生产部署验证 |
...
| identity | admin auth verified + G2A-03 local verified candidate | 管理认证已补救复验；最终用户存储、认证/账号 Service、13 条 HTTP 路由、refresh 重放撤销、并发幂等和稳定错误已通过本地真实 PostgreSQL 与 Full | G2A-03 托管 CI；G2A-04 Provider；绑定/合并完整流程 |
| product-user-access / account composition | G2A-03 local verified candidate | 独立 Product/Tenant 准入事实、Guard 管理 API、审计/撤销 Outbox、实时组合裁决和登录前准入已通过本地真实 PostgreSQL 与 Full | G2A-03 托管 CI、ST-038 后续装配验收与 entitlement 组合 |
...
- G2A-03：两轮审查发现的 4 个 P1 已修复；本地真实 PostgreSQL、OpenAPI 89/95、Full 18/18、SDK 8/8、Client UI 14/14、Standard-A 双目标各 7/7、Admin 133/133 均通过。本地提交尚未上传，状态保持 `local_verified_ci_pending`，证据见 `artifacts/reviews/G2A-03/user-auth-account-api.md`。

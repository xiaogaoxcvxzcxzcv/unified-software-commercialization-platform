...
| 产品蓝图与 Assembly | G1-05/G1-07/G1-07.1/G1-08.1 verified | G1-04 后端基础、确定性 Generator、Standard-A 模板、受信工具目录基础、创建 API Client 与状态模型已通过真实 PostgreSQL Full 门禁；工具清单、执行入口和输出目标由服务端锁定，浏览器只获得脱敏引用；四个工具目录当前为空 | 当前 G1-08.2 `/create` 多步向导；随后软件工作区、装配恢复、lifecycle API、可信扩展目录 |
...
| 管理后台 | demo + auth/G1-08.1 verified | React + TypeScript 可运行工程；平台/产品双上下文；管理员登录补救复验通过；Assembly 创建 API Client/状态模型、401 原请求重放、脱敏输出目标和 69 项 Vitest/生产构建通过 | `/create` 页面、其余真实业务 API Client、业务页面 E2E、平板视觉 QA |
...
| 模块 | 当前状态 | 说明 |
|---|---|---|
| assembly | G1-05/G1-07/G1-07.1/G1-08.1 verified | 后端、Generator 文件安全、`standard-a` 视觉、受信工具目录基础、脱敏输出目标 API、管理 Client 与状态模型已验证；普通/实验 scope、快照和环境绑定失败关闭；普通包、模板和真实工具版本仍为空 | 当前 G1-08.2 向导；随后软件工作区、装配恢复、lifecycle API、扩展目录；G2C 发布真实工具并装配样板 |

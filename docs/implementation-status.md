...
| 完整能力包目录 | package.account contracted | Account 独立 Manifest、配置 Schema、九面合同、Product User Access 边界和目录防泄漏已验证；G2A-03/G2A-04/G2A-04.1 已完成认证账号 API、准入、外部身份、安全通知和 HostedInteraction 后端；G2A-05 管理 Blocks 已验证；G2A-06 用户 Blocks 已完成本地验收并等待托管 required check；普通与实验能力包目录仍为空 | G2A-07 SDK/配置/源码、G2A-08 包内验证和 G2C 装配回归；当前没有 verified/available 包 |
...
| 管理后台 | auth/G1-08.1-G1-08.4/G1-10/G2A-05 verified | 生命周期 Plan/Operation、重新认证回跳、幂等冲突显式处理、工件验证和有界轮询已实现；G2A-05 Account API Client、真实后端、权限/危险操作、nullable 会话、能力集审计持久化和浏览器入口已通过真实 PostgreSQL、浏览器与托管 required check。`identity.user-table`、`identity.user-detail` 机器目录已同步为 ready；`package.account` 仍为 `contracted` | G2A-07 SDK/配置/源码、G2A-08 九面验证；平板视觉 QA |
...
| Client UI / Hosted UI | G1-06/G1-07 verified + G2A-06 local verified pending CI | `@capability-platform/client-ui` v0.1.0 与 `standard-a` 已验证；6 个 Account 用户 Block、Hosted auth/account 编排、响应式/深色主题、可访问性、密码清理、active-only 会话、真实 HTTPS 浏览器和 no-store/no-CORS 响应头已通过本地 Full 20/20，Client UI 123/123、Hosted 52/52 | 当前提交的托管 required check；G2A-07 SDK/配置/源码、小程序/原生适配和 G2A-08/G2C 装配回归 |
...
| identity | admin auth + G2A-04 external identity + G2A-04.1 hosted binding + G2A-05 admin verified | 管理认证、最终用户认证/账号、refresh 重放撤销、外部身份、注册验证/恢复投递、Hosted proof/grant 和管理 Blocks 已通过真实 PostgreSQL 与托管 CI；G2A-06 又验证了 active-only self-service session 投影、密码全路径清理和真实 Hosted 浏览器流程 | 当前 G2A-06 托管 required check；生产 OIDC/微信 Provider E2E；G2A-07/G2A-08 |
| hosted-interaction | G2A-04.1 verified + G2A-06 local verified pending CI | ADR-0018 所有权、`000018` 至 `000025`、interaction/browser session/completion grant、自助 profile/password/session 流、精确 return target、state/nonce/PKCE、一次性 code、Origin/CSRF 和幂等恢复已实现；G2A-06 本地真实 PostgreSQL、HTTPS 浏览器、响应头与 Full 20/20 已通过 | 当前提交的托管 required check；G2A-07 SDK/配置/源码、G2A-08 包内九面验证 |
...
- G2A-06：本地验收通过，等待当前提交的托管 required check。6 个 Account 用户 Block、Hosted auth/account 编排、active-only session、密码字段全路径清理、Origin/CSRF/CORS 失败关闭、真实 HTTPS 浏览器、多视口和稳定完成状态已验证；Full `-RequirePostgres` 20/20、OpenAPI 118/124、Client SDK 8/8、Client UI 123/123、Admin 158/158、Hosted 52/52 和全部生产构建通过。证据：`artifacts/reviews/G2A-06/browser-acceptance.md`、`artifacts/reviews/G2A-06/quality-gate-full-postgres-g2a06-final.json`。G2A-07 保持 `planned`，`package.account` 仍为 `contracted` 且没有 available 包。

...
| 完整能力包目录 | package.account contracted | Account 独立 Manifest、配置 Schema、九面合同、Product User Access 边界和目录防泄漏已验证；G2A-03 至 G2A-07 的认证账号 API、准入、外部身份、安全通知、HostedInteraction、管理 Blocks、用户 Blocks、SDK/配置/Generated Source 均已 verified；普通与实验能力包目录仍为空 | G2A-08 包内九面验证正在推进；仍缺机器证据、experimental verified candidate 目录发布、托管 CI 和 G2C 装配回归；当前没有 verified/available 完整能力包 |

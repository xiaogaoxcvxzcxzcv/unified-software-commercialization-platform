...
| Go 后端 | G1 + G2A + G2B-02 verified | Assembly 发布闭包，以及 Identity 最终用户、认证/账号 API、外部身份、注册/恢复安全投递和 Product User Access 已通过真实 PostgreSQL、本地 Full 与托管 push/PR CI；HostedInteraction 登录/账号后端、Identity proof/grant 会话绑定和真实运行时 HTTP + PostgreSQL 流已验证；Entitlement `000026`、Domain/Application/PostgreSQL Adapter、HTTP API、Outbox/Audit、幂等、复杂策略和 source tuple revoke 已通过真实 PostgreSQL Full 与托管 required check | 后续 Entitlement 管理后台、用户前台、SDK、完整能力包和生产部署验证 |
...
- G2B-03：`in_progress`。已补齐 Entitlement 管理查询契约、OpenAPI、后端 `ListCurrentEntitlements`、真实管理 API Client、`/products/:productId/entitlements` 路由和页面，替换原演示 Client；页面支持服务端范围查询、授予、延长、撤销、Ledger 历史、幂等键、审计编号、撤销二次确认和 reauth 后返回当前页。浏览器验收发现并补齐 Access Control 中缺失的 `entitlement.read` 与 `entitlement.revoke` 权限目录。本地 `go test ./internal/modules/accesscontrol ./internal/modules/entitlement/... ./cmd/server`、OpenAPI 123/130、Admin 165/165、Admin build、Core 6/6、Full `-RequirePostgres` 22/22 和真实浏览器授予/流水/延长/撤销主路径已通过；尚缺提交、push 和 GitHub required check，因此 `entitlement.table`、`entitlement.grant-panel`、`entitlement.history` 仍不得标为 ready，G2B-04 仍不得开始。证据草案：`artifacts/reviews/G2B-03/entitlement-admin-blocks.md`。

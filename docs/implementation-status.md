...
| 管理后台 | auth/G1-08.1-G1-08.4/G1-10/G2A-05 verified | 生命周期 Plan/Operation、重新认证回跳、幂等冲突显式处理、工件验证和有界轮询已实现；G2A-05 Account API Client、真实后端、权限/危险操作、nullable 会话、能力集审计持久化和浏览器入口已通过专用验收夹具、真实 PostgreSQL Full 18/18、管理后台 149/149、生产构建及 push/PR required quality-gate。`package.account` 仍为 `contracted`，完整能力包九面和普通创建仍未完成 | G2A-06 Hosted UI、G2A-07 SDK/配置/源码、G2A-08 九面验证；平板视觉 QA |

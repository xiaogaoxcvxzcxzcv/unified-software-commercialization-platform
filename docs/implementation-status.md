...
| 完整能力包目录 | package.account verified candidate / package.entitlement contracted | Account 独立 Manifest、配置 Schema、九面合同、Product User Access 边界和目录防泄漏已验证；G2A-03 至 G2A-08 的认证账号 API、准入、外部身份、安全通知、HostedInteraction、管理 Blocks、用户 Blocks、SDK/配置/Generated Source、包内九面验证和 experimental candidate 发布均已通过；Entitlement G2B-01 模型、Manifest、唯一约束和并发规则已封口；G2B-02 后端、迁移、HTTP API、Outbox、复杂策略和真实 PostgreSQL 验证已通过；G2B-03 管理后台 Blocks 与 G2B-04 用户前台/SDK/源码已验证；ordinary 目录仍为空 | 下一唯一关口为 G2B-05 包内九面验证；仍缺 G2B-05 和 G2C 装配回归，当前没有 available 完整能力包 |
...
| 管理后台 | auth/G1-08.1-G1-08.4/G1-10/G2A-05/G2B-03 verified | 生命周期 Plan/Operation、重新认证回跳、幂等冲突显式处理、工件验证和有界轮询已实现；G2A-05 Account API Client、真实后端、权限/危险操作、nullable 会话、能力集审计持久化和浏览器入口已通过真实 PostgreSQL、浏览器与托管 required check；G2B-03 Entitlement API Client、查询、授予、延长、撤销、流水、reauth 返回和浏览器主路径已通过本地 Full 与托管 required check。`identity.user-table`、`identity.user-detail`、`entitlement.table`、`entitlement.grant-panel`、`entitlement.history` 已同步为 ready；`package.account` 已完成 G2A-08 experimental verified candidate | 后续 G2B-05 Entitlement 包内九面验证和 G2C 装配回归 |
...
| entitlement | G2B-04 verified；package still contracted | G2B-01 已补齐 Feature/Policy/Validity/Grant/Revision/Ledger/Check Decision、叠加/撤销/到期语义、唯一约束、幂等和并发策略；G2B-02 已完成 `000026`、公开应用服务、PostgreSQL Adapter、HTTP API、Outbox/Audit、同 source/effect 与幂等去重、expected revision、source tuple revoke、互斥/优先级/replace/reject conflict 和真实 PostgreSQL/Full/CI 验证；G2B-03 管理查询 API、真实管理 API Client、Entitlement 管理 Blocks 和浏览器主路径已 verified；G2B-04 用户前台、SDK、Hosted account 集成、Generated Source、能力禁用边界、到期空态和专用真实浏览器 E2E 已 verified；本地 Full `-RequirePostgres` 22/22 通过 | 仍缺 G2B-05 包内九面和 G2C 装配回归 |
...
- G2B-04：`verified`。`entitlement.summary` 用户前台 Block、Entitlement TypeScript SDK、Hosted account 可选权益投影、Generated Source、包内容树测试、能力禁用失败关闭边界和 OpenAPI 投影已完成；补齐 Hosted account 对空 features + 过期 `valid_until` 的到期空态映射，避免通用账号空态覆盖“权益已到期”。专用真实浏览器 E2E 在 Hosted Web 中打开 4 个 `hosted.account` interaction，覆盖有权益、无权益、到期和能力禁用；有权益页显示 `权益摘要`、`pro`、`priority_queue`，且不展示价格、支付状态或金额字段，禁用时不显示“当前权益”入口。本地 Full `-RequirePostgres` 22/22 通过，报告为 `artifacts/reviews/G2B-04/quality-gate-full-postgres-browser-e2e.json`；Hosted Web 57/57、Client UI 129/129、SDK 43/43、Admin 165/165 和生产构建均通过。`entitlement.summary` 实现已 verified，但运行时 Feature Block 目录仍保持 `not_ready`，等待 G2B-05 包内九面验证后再提升；`package.entitlement` 仍只是 contracted，下一唯一关口为 G2B-05。证据：`artifacts/reviews/G2B-04/entitlement-user-sdk-source.md`。

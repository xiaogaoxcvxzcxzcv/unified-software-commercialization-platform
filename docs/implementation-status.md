...
| entitlement | G2B-03 verified；G2B-04 implemented_local_full_passed；package still contracted | G2B-01 已补齐 Feature/Policy/Validity/Grant/Revision/Ledger/Check Decision、叠加/撤销/到期语义、唯一约束、幂等和并发策略；G2B-02 已完成 `000026`、公开应用服务、PostgreSQL Adapter、HTTP API、Outbox/Audit、同 source/effect 与幂等去重、expected revision、source tuple revoke、互斥/优先级/replace/reject conflict 和真实 PostgreSQL/Full/CI 验证；G2B-03 管理查询 API、真实管理 API Client、Entitlement 管理 Blocks 和浏览器主路径已 verified；G2B-04 已补用户前台、SDK、Hosted account 集成、Generated Source 和能力禁用边界，本地 Full `-RequirePostgres` 22/22 通过；仍缺 G2B-04 专用真实浏览器 E2E、提交/PR/CI 和 G2B-05 包内九面 |

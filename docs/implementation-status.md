...
| 完整能力包目录 | package.account contracted | Account 独立 Manifest、配置 Schema、九面合同、Product User Access 边界和目录防泄漏已验证；G2A-03 已完成用户认证/账号 API、准入管理与组合裁决；G2A-04 已完成外部身份与安全通知后端基础；普通与实验能力包目录仍为空 | G2A-04.1 HostedInteraction、G2A-05/G2A-06 UI/SDK 等其余交付面；当前没有 verified/available 包 |
...
| Go 后端 | G1 + G2A-04 verified | Assembly 发布闭包，以及 Identity 最终用户、认证/账号 API、外部身份、注册/恢复安全投递和 Product User Access 已通过真实 PostgreSQL、本地 Full 与托管 push/PR CI | G2A-04.1 HostedInteraction、后续 Account/Entitlement 完整能力包和生产部署验证 |
...
| identity | admin auth + G2A-04 external identity verified | 管理认证已补救复验；最终用户认证/账号、refresh 重放撤销、外部身份 start/callback/微信一次性交换、绑定/查询/解绑、注册验证与恢复投递已通过真实 PostgreSQL、本地 Full 与托管 CI | 生产 OIDC/微信 Provider E2E；G2A-04.1 HostedInteraction 后端；G2A-05/G2A-06 管理/用户 UI |

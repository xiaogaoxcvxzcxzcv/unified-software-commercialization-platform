...
| 完整能力包目录 | package.account contracted | Account 独立 Manifest、配置 Schema、九面合同、Product User Access 边界和目录防泄漏已验证；G2A-03 已完成用户认证/账号 API、准入管理与组合裁决；G2A-04 已完成外部身份与安全通知后端基础；G2A-04.1 HostedInteraction 登录/账号后端已完成本地 Full、确定性并发专项与托管 push/PR 验证；普通与实验能力包目录仍为空 | G2A-05 至 G2A-08 其余交付面；当前没有 verified/available 包 |
...
| 管理后台 | auth/G1-08.1-G1-08.4/G1-10 verified；G2A-05 in_progress | 生命周期 Plan/Operation、重新认证回跳、幂等冲突显式处理、工件验证和有界轮询已实现；G2A-05 Account API Client、真实后端、权限/危险操作、nullable 会话和浏览器入口已通过专项与 Full 门禁；当前缺少启用 `package.account` 的真实 Product，正向用户表/详情浏览器点击仍待补 | 真实 Product Account 正向浏览器 E2E、平板视觉 QA、PR/托管 CI |
...
| Client UI / Hosted UI | G1-06 foundation + G1-07 template verified | `@capability-platform/client-ui` v0.1.0 基座已验证；`standard-a` 已完成 Web/desktop WebView 生成、custom 路由、响应式 Shell、主题、离线安装、构建、浏览器视觉与键盘验收；真实 HostedInteraction 登录/账号后端已通过 G2A-04.1 验证 | 业务 Feature Block、小程序/原生适配、Hosted UI 页面和浏览器/跨端 E2E |
...
| identity | admin auth + G2A-04 external identity + G2A-04.1 hosted binding verified | 管理认证已补救复验；最终用户认证/账号、refresh 重放撤销、外部身份 start/callback/微信一次性交换、绑定/查询/解绑、注册验证与恢复投递已通过真实 PostgreSQL、本地 Full 与托管 CI；Hosted proof/grant 的环境、会话绑定、一次性兑换和真实运行时登录/账号链已通过 G2A-04.1 本地与托管验证 | 生产 OIDC/微信 Provider E2E；G2A-05/G2A-06 管理/用户 UI |
| hosted-interaction | G2A-04.1 verified | ADR-0018 所有权、`000018` 至 `000022`、interaction/browser session/completion grant、精确 return target、state/nonce/PKCE、一次性 code、CSRF、租约接管、恢复与取消/过期已实现；修复提交 `eb89c1d` 通过本地真实 PostgreSQL Full 18/18、确定性并发专项 `-count=3`、push run `29626935922` 和 PR run `29626937426`，机器报告提交为 `35b38d6` | G2A-06 Hosted UI、G2A-07 SDK/配置/源码、G2A-08 包内九面验证 |
...
- G2A-04.1：`verified`。ADR-0018、`000018` 至 `000022`、7 条 Hosted 路由、Identity proof/grant 会话绑定，以及真实运行时 auth/account HTTP + PostgreSQL 链已实现；修复提交 `eb89c1d` 的本地真实 PostgreSQL Full 18/18 与 Hosted 确定性并发专项 `-count=3` 均通过，机器报告提交 `35b38d6` 引用该修复提交。GitHub push run `29626935922` 与 PR run `29626937426` 均成功；历史 PR run `29626127011` 的短 TTL 失败及修复过程已记录。证据：`artifacts/reviews/G2A-04.1/hosted-interaction-auth-account.md`。当前唯一关口切换为仍处于 `planned`、尚未开始的 G2A-05；`package.account` 仍为 `contracted`，当前没有 `available` 完整能力包。

...
| entitlement | G2B-02 backend/API verified；package still contracted | G2B-01 已补齐 Feature/Policy/Validity/Grant/Revision/Ledger/Check Decision、叠加/撤销/到期语义、唯一约束、幂等和并发策略；G2B-02 已完成 `000026`、公开应用服务、PostgreSQL Adapter、HTTP API、Outbox/Audit、同 source/effect 与幂等去重、expected revision、source tuple revoke、互斥/优先级/replace/reject conflict 和真实 PostgreSQL/Full/CI 验证；仍未完成管理 Blocks、用户前台、SDK/源码和包内九面 |

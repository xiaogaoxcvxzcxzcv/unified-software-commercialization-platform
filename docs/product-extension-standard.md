...
Extension 是装配完成后该软件自己的开发机制，不表示统一底座要继续开发这款软件的正文。底座主计划只使用最小 Extension 夹具验证路由、后台入口、公开服务、数据隔离和升级兼容；真实业务首页、目录、工作台、流程和数据模型必须在该软件自己的独立开发任务中实现。
...
当另一款真实软件也需要同类扩展时，先检查能力索引和完整能力包目录。确认具有稳定共性后，另立平台能力变更，补齐九个交付面、兼容与回归；不能直接把某款软件的独有代码复制到共享平台。

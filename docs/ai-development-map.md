...
- 当前阶段：F0、G1、G2A-01 至 G2A-07 已退出；`package.account` 仍为 contracted。G2A-07 Account SDK、配置 Schema、Manifest 内容和 Generated Source 已通过本地真实 PostgreSQL Full 22/22、提交 `5b49f6d10225402b3ae448b042e2b2f060a6ead6` 和 GitHub required check。当前唯一严格关口为 `in_progress` 的 G2A-08 Account 包内九面验证；真实工具/能力包版本尚未发布，普通创建入口继续真实空状态并失败关闭。
- 正式代码目录：`platform/`。
...
- Assembly 后端执行/恢复闭包、TypeScript SDK/Client UI 基座、`standard-a` 实验模板候选、创建向导、单款软件工作区和 lifecycle API 已实现；lifecycle 已通过本地与托管 CI 验证。Account 管理端两个 Block 与用户端六个 Block 已 verified；Account SDK 的 22 个公开方法、可注入安全 Vault、配置 Schema 和 Generated Source 已实现并通过 G2A-07 本地专项、Full 和托管 CI 验证。G2A-08 包内九面已开始冻结验收与证据口径，首个装配验收软件仍未开始，当前仍不能声称“勾选能力即可得到完整前后台”。

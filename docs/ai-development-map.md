...
- 当前阶段：F0、G1、G2A-01 至 G2A-06 已退出；`package.account` 仍为 contracted。当前唯一关口 G2A-07 为 `in_progress`：Account SDK、配置 Schema、Manifest 内容和 Generated Source 已在本地脏工作区通过 Full `-RequirePostgres` 22/22，提交和托管 CI 尚未完成。G2A-08 保持 `planned`；真实工具/能力包版本尚未发布，普通创建入口继续真实空状态并失败关闭。
- 正式代码目录：`platform/`。
...
- Assembly 后端执行/恢复闭包、TypeScript SDK/Client UI 基座、`standard-a` 实验模板候选、创建向导、单款软件工作区和 lifecycle API 已实现；lifecycle 已通过本地与托管 CI 验证。Account 管理端两个 Block 与用户端六个 Block 已 verified；Account SDK 的 22 个公开方法、可注入安全 Vault、配置 Schema 和 Generated Source 已实现并通过本地专项验证，但 G2A-07 尚待提交与托管 CI，G2A-08 包内九面和首个装配验收软件仍未开始，当前仍不能声称“勾选能力即可得到完整前后台”。

...
当前唯一严格关口：G1-08.1 创建软件 API Client 与状态模型
F0-02 已重新达标：首次失效历史保留，补救后的 PostgreSQL、浏览器和 Full 证据支持 `verified_after_remediation`
G1-07/G1-07.1 已通过；后续严格顺序：G1-08.1 API Client/状态模型 -> G1-08.2 创建向导 -> G1-08.3 软件工作区 -> G1-08.4 装配记录/恢复 -> G1-10 lifecycle API -> G1-11 Extension Catalog

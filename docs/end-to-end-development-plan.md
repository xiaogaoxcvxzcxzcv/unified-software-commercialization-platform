...
状态：`in_progress`（2026-07-23）。已补齐管理端查询契约：`GET /api/v1/admin/entitlements` 返回当前 Revision 投影分页，`GET /api/v1/admin/entitlements/history` 返回 Ledger 分页；后端、OpenAPI、管理后台真实 API Client、`/products/:productId/entitlements` 路由和页面已实现。本地受影响 Go、OpenAPI、Admin 专项/全量、Admin build、Core 与 Full `-RequirePostgres` 通过。尚缺真实浏览器验收、提交、push 和 GitHub required checks，因此不得标记 verified，也不得进入 G2B-04。

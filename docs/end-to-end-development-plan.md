...
状态：`in_progress`（2026-07-22）。本关先完成九面验收冻结、证据矩阵、双 Product/双 Tenant 隔离与失败恢复验证设计；未完成全部机器证据、Full 和最终托管 CI 前不得标记 verified。`package.account` 源契约仍为 `contracted`，ordinary runtime catalog 仍为空；experimental runtime catalog 已有正在验收的 G2A-08 candidate。

...
状态：`in_progress`（2026-07-15）。G1-07 已 verified，本工作包成为当前唯一严格关口；尚未把任何工具登记到 ordinary 或 experimental 目录。
...
当前唯一严格关口：G1-07.1 受信 Generator/SDK 工具目录
F0-02 已重新达标：首次失效历史保留，补救后的 PostgreSQL、浏览器和 Full 证据支持 `verified_after_remediation`
G1-07 已通过；后续严格顺序：G1-07.1 受信工具目录 -> G1-08 创建向导与软件管理工作区 -> G1-10 lifecycle API -> G1-11 Extension Catalog

...
状态：`local_verified_pending_status_commit_ci`（2026-07-23）。已发布 `package.account` + `package.entitlement` + `standard-a` + `platform.generator` + `platform.sdk` + `extension.editor-tools` 的 experimental 组合闭包；真实机器目录和 Planner 测试覆盖依赖解析、确定性计划、工具/扩展锁定、ordinary 不可见与非法组合失败关闭。本地 Full `-RequirePostgres` 22/22 已通过，报告为 `artifacts/reviews/G2C-01/quality-gate-full-postgres.json`；代码 checkpoint 远端提交 `8509c3e76e46588430a017a3045da18e757baad1` 的 push/PR required checks 已通过。待本次状态同步提交完成远端 required checks 后，本关口才能最终标记 `verified` 并进入 G2C-02。
...
G1-07/G1-07.1/G1-08.1/G1-08.2/G1-08.3/G1-08.4/G1-10/G1-11/G2A-01/G2A-02/G2A-03/G2A-04/G2A-04.1/G2A-05/G2A-06/G2A-07/G2A-08/G2B-01/G2B-02/G2B-03/G2B-04/G2B-05 已通过；当前唯一严格关口为 G2C-01 组合锁定与实验入口预检。`package.account` 与 `package.entitlement` 仅为 experimental verified candidate，G2C-01 已完成本地 Full 22/22 和代码 checkpoint 远端 required checks，仍待状态同步提交的远端 required checks 后才能进入 G2C-02；ordinary 目录仍没有 `available` 完整能力包，不得跳过后续交付面。

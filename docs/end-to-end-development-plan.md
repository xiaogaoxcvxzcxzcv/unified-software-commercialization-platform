...
状态：`in_progress`（2026-07-15）。`standard-a` 0.1.0 已进入受控实验模板目录；Web/desktop WebView 真实生成、custom 工作台独立注入、离线安装、交互测试、生产构建和本机 HTTP 启动通过。F0 已退出，当前唯一工作是模板自身的桌面、窄屏和移动截图视觉 QA、交互及可访问性证据；完成前不得进入 G1-07.1。证据：`artifacts/reviews/G1-07/standard-a-template.md`。
...
当前唯一严格关口：G1-07 第一套 Web/桌面模板视觉收口
F0-02 已重新达标：首次失效历史保留，补救后的 PostgreSQL、浏览器和 Full 证据支持 `verified_after_remediation`
G1-07 通过后：G1-07.1 受信工具目录 -> G1-08 创建向导与软件管理工作区 -> G1-10 lifecycle API -> G1-11 Extension Catalog

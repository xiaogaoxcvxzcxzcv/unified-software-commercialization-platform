...
状态：`in_progress`（2026-07-15）。本地 `Full -RequirePostgres` 已扩展并通过全部 18 项；GitHub Actions 已配置同一入口和 PostgreSQL 17 service。首次托管运行已执行并暴露 Linux 路径硬编码与 npm 10 离线 peer 解析问题，修复已在本地等价流程通过，待提交推送后取得绿色托管运行。仓库为私有且当前 GitHub 套餐拒绝配置 required check，不能在未经授权时改为公开仓库。报告：`artifacts/reviews/F0-03/quality-gate-report.json`。
...
当前唯一严格关口：F0-03 GitHub Actions 绿色托管运行与 required check

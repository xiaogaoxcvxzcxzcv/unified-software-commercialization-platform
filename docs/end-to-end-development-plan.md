...
状态：`in_progress`。本地实现与验收已通过：6 个 Account 用户 Block、Hosted auth/account 编排、真实 HTTPS 浏览器流程、active-only 会话投影、密码字段全路径清理、Origin/CSRF/CORS 失败关闭、Full `-RequirePostgres` 20/20、Client UI 123/123、Admin 158/158、Hosted 52/52 及全部生产构建均为绿色。证据见 `artifacts/reviews/G2A-06/browser-acceptance.md` 和 `artifacts/reviews/G2A-06/quality-gate-full-postgres-g2a06-final.json`。当前仅等待该提交的托管 required check；通过后才改为 `verified` 并切换 G2A-07。`package.account` 仍为 `contracted`。
...
状态：`planned`。G2A-06 未经当前提交的托管 required check 裁决前不得开始。
...
G1-07/G1-07.1/G1-08.1/G1-08.2/G1-08.3/G1-08.4/G1-10/G1-11/G2A-01/G2A-02/G2A-03/G2A-04/G2A-04.1/G2A-05 已通过；当前唯一严格关口为 `in_progress` 的 G2A-06 用户前台 Hosted UI。本地真实 PostgreSQL、浏览器、多视口、响应头、Full 20/20 和前后端构建已通过，等待当前提交的托管 required check 后裁决。G2A-07 必须保持 `planned`；`package.account` 仍为 `contracted`，不得跳过后续交付面。

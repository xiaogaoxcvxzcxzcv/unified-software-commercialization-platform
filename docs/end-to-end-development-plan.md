...
状态：`implemented_local_remote_ci_passed`（2026-07-23）。`entitlement.summary` 用户前台 Block、Entitlement TypeScript SDK、Hosted account 可选权益投影、生成源码模板、包内容树测试、能力禁用失败关闭边界和 OpenAPI 投影已实现；本地真实 PostgreSQL Full `-RequirePostgres` 22/22 通过，报告为 `artifacts/reviews/G2B-04/quality-gate-full-postgres-local.json`。提交 `7317210` 已 push；push run `29992876224` 与 pull_request run `29993171659` 的 `quality-gate` 和 `windows-tls` 均成功。本状态不是 `verified`：仍缺 G2B-04 专用真实浏览器 E2E；G2B-05 必须保持 `planned`，`package.entitlement` 仍为 `contracted`，不得进入 experimental candidate 或 ordinary available。
...
G1-07/G1-07.1/G1-08.1/G1-08.2/G1-08.3/G1-08.4/G1-10/G1-11/G2A-01/G2A-02/G2A-03/G2A-04/G2A-04.1/G2A-05/G2A-06/G2A-07/G2A-08/G2B-01/G2B-02/G2B-03 已通过；当前唯一严格关口为 G2B-04 用户前台、SDK 和源码。`package.account` 仅为 experimental verified candidate，`package.entitlement` 仍为 contracted，G2C 仍为 `planned`，不得跳过后续交付面。

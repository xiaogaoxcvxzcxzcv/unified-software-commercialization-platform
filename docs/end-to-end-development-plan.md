...
状态：`in_progress`（2026-07-17）。当前契约与 ADR-0015 已冻结：扩展在 Product 创建前绑定 Blueprint 的稳定 `product.code`，Product 创建后由服务端装配链绑定生成的 `product_id`；客户端 `manifest_path` 只做规范路径比对，不能成为文件读取入口。

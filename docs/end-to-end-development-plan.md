...
状态：`in_progress`（2026-07-22）。本关先完成九面验收冻结、证据矩阵、双 Product/双 Tenant 隔离与失败恢复验证设计；未完成机器证据、目录发布和托管 CI 前不得标记 verified。`package.account` 仍为 `contracted`，ordinary 与 experimental runtime catalog 仍为空。

...
状态：`in_progress`（2026-07-18）。Identity 范围成员查询、Product User Access 批量覆盖、Account 组合工作流、全局安全状态、管理员会话撤销、精确审计读取和服务端能力启用边界已实现；nullable 会话环境、全局 Identity 错误映射和审计轮询取消已补强。真实浏览器已通过管理员登录、Overview/API 连接、真实 Product 空态、失败关闭和 1280/390 视口验收；但当前本机没有启用 `package.account` 的真实 Product，正向 user-table/user-detail 点击验收尚缺，故不得标记 verified。`identity.user-table`、`identity.user-detail` 仍为 `not_ready`，`package.account` 仍为 `contracted`。
...
G1-07/G1-07.1/G1-08.1/G1-08.2/G1-08.3/G1-08.4/G1-10/G1-11/G2A-01/G2A-02/G2A-03/G2A-04/G2A-04.1 已通过；G2A-04.1 的本地真实 PostgreSQL Full 18/18、Hosted 确定性并发专项、push run `29626935922` 和 PR run `29626937426` 均通过；当前唯一严格关口为 `in_progress` 的 G2A-05 统一后台 Account Blocks，当前处于契约冻结阶段

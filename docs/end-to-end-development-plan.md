...
7. G2C-02/G2C-04 只允许通过服务端授权的 experimental 入口验证候选；完成双产品回归并晋级后，G2C-05 必须再从普通入口完成一次无 experimental 权限的软件创建终验。
...
| 26 | G2C-02 | 从受控 experimental 入口真实点击创建第一款软件 | 服务端授权候选目录和 `output_target_ref`；自动建立 Product/Tenant/Application/凭据，启用共享后端，并在隔离源码根/制品根产生 Manifest、lock、SDK 配置和可运行前台源码 | G2C-03 |
| 27 | G2C-03 | 同一次创建的两个可见界面 | 创建后该软件后台出现用户/权益管理；用户前台出现登录/个人中心/权益；两边读取同一真实事实，ST-028 通过 | G2C-04 |
| 28 | G2C-04 | 禁用、失败恢复、重生成、升级/回滚和独有扩展 | ST-029/030/031/032 全部通过；custom 业务不被覆盖；失败不会留下半装配状态 | G2C-05 |
| 29 | G2C-05 | 第二款软件、目录晋级与普通入口终验 | 候选 Product B 创建后 A 回归和 ST-033 通过；account/entitlement/standard-a 晋级 ordinary `available`；无 experimental 权限从普通入口创建 Product C，两个界面和黄金流再次通过 | G3 |
...
状态：`in_progress`（2026-07-15）。本地、push 运行 `29403678552` 和 PR 运行 `29403976845` 的 `Full -RequirePostgres` 均通过全部 18 项；GitHub Actions 使用 Ubuntu、PostgreSQL 17、Node.js 22 和同一仓库入口。失败运行先后暴露的 Linux 路径、npm 10 peer 解析和 Standard-A 离线 registry metadata 缓存问题均已修复。唯一剩余验收是 required check：仓库为私有且当前 GitHub 套餐拒绝 branch protection/ruleset 写入，不能在未经授权时改为公开仓库。证据：`artifacts/reviews/F0-03/hosted-quality-gate.md`、`artifacts/reviews/F0-03/quality-gate-hosted.json`。
...
状态：`implemented`（2026-07-15 更新）。`standard-a` 0.1.0 已进入受控实验模板目录；Web/desktop WebView 真实生成、custom 工作台独立注入、离线安装、交互测试、生产构建和本机 HTTP 启动通过。Codex 内置浏览器的本机访问已恢复，但模板自身的桌面/窄屏截图视觉 QA 尚未执行，因此尚未提升为工作包 `verified`。证据：`artifacts/reviews/G1-07/standard-a-template.md`。
...
交付：只读 Extension Catalog、Manifest 摘要与版本校验、Product 绑定、前台 route/navigation/slot/event、后台目录项、权限、公开 API/事件、数据命名空间、安装/卸载和数据保留计划。

验收：未知、跨 Product、权限缺失、路径冲突、跨模块表访问和产品名硬编码全部拒绝；目录本身可独立验证，带真实软件的安装、升级和 ST-032 整体留到 G2C。
...
### G2C-02 统一后台真实创建第一款软件

交付：具有服务端实验权限的管理员从 `/create` 的受控 experimental 入口选择 Web/桌面、account、entitlement 和 standard-a，选择服务端授权的 opaque `output_target_ref`，审阅计划并点击创建；Assembly 自动建立 Product、official Tenant、Application、测试凭据、CapabilitySet、Manifest 和 lock，并把源码与机器证据写入互不重叠的受控根。

验收：普通入口此时仍不能看见候选包；前端参数、蓝图字段或普通管理员不能开启 experimental 目录；未知、越权、改换或映射重叠的 `output_target_ref` 均拒绝。刷新和响应中断后可恢复同一 Run；重复点击不重复创建事实；失败留下脱敏诊断和恢复位置；成功后源码根与制品根证据闭包一致，并自动进入该 Product 的统一后台工作区。

### G2C-03 同一次创建的后台与用户前台

交付：统一后台自动注册真实 Account/Entitlement Blocks；生成软件得到可运行 Shell、登录、个人中心、权益页面、SDK 配置和 custom 工作台扩展。

验收：ST-028 完整通过；用户真实注册/登录，管理员在该软件后台查询用户并授予权益，SDK 与个人中心显示同一真实结论；没有手写登录、个人中心或权益状态机。

### G2C-04 失败、禁用、重生成、升级与独有扩展

交付：能力禁用/恢复、网络与进程失败恢复、重复生成、模板/包升级回滚、eject 和样板独有前台/后台/数据扩展。

验收：ST-029、ST-030、ST-031、ST-032 全部通过；前台、后台和 API 的禁用行为一致；custom 不覆盖；升级失败可恢复；扩展不能越权或跨模块访问表。

### G2C-05 第二款软件、目录晋级与普通入口终验

交付：先用同一候选组合从受控 experimental 入口创建 Product B，记录 Product A 的 Manifest、lock、API、数据和黄金流基线并完整回归。通过全部候选门槛后，把 `package.account`、`package.entitlement` 和 `standard-a` 仅针对已验证目标端、交付形态和环境晋级 ordinary `available`；随后移除当前管理员的 experimental 权限，从普通 `/create` 创建 Product C。

验收：ST-033 通过；B 只新增蓝图、配置、凭据、生成物和 custom 代码，A 不变。晋级操作只接受锁定证据并留下审计，不能由前端直接改 readiness；普通目录随后只显示精确 `available` 组合。Product C 的请求和蓝图不含 experimental 开关，仍能建立完整 Product/Tenant/Application、生成源码/证据、进入自己的统一后台工作区，并再次跑通 G2C-03 的用户注册登录、管理员授予权益和用户前台权益显示。三款产品互相隔离、A/B 回归不变后，G2 才完成并进入 G3。
...
当前唯一严格关口：F0-03 required check 套餐/仓库可见性裁决与落地；push 和 PR 托管质量门已绿色
F0-02 已重新达标：首次失效历史保留，补救后的 PostgreSQL、浏览器和 Full 证据支持 `verified_after_remediation`
F0-03 与 G1-07 全部通过后：G1-07.1 受信工具目录 -> G1-08 创建向导与软件管理工作区 -> G1-10 lifecycle API -> G1-11 Extension Catalog
G1-09 已取消独立执行并合并到 G2C；不得创建空包或假包绕过 Product Blueprint 至少一个能力包的机器契约
当前没有 available 完整能力包
```

后续 AI 或开发者不得跳过当前严格关口直接堆 Account 页面，也不得在 Account/Entitlement 未通过 G2C 前横向开始支付、AI、存储或更多后台菜单。任一项未达到表 6.2 的验收标准时，下一项保持 `planned`。

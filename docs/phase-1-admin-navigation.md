...
本文确定第一阶段实际显示的后台目录。它是页面、路由、权限、Feature Block 和后端能力的共同约束，不只是界面草图。
...
后续能力完成并通过冒烟测试后，再加入当前软件左侧栏：

| 阶段 | 新增目录 |
|---|---|
| 阶段 2 | 设备、激活码 |
| 阶段 3 | 商品套餐、订单、支付退款、对账 |
| 阶段 4 | 版本发布、远程配置、公告与二维码 |
| 阶段 5 | 文件存储、用量额度、通知、运营数据 |
| 阶段 6 | 优惠券、发票、分销、佣金、白标等确认有需求的能力 |

未进入当前阶段的目录不显示“敬请期待”占位项，避免后台看起来很全但实际不可用。

...
`package.account` 仍处于 `contracted`。G2A-03/G2A-04 已交付最终用户认证账号 API、范围准入、外部身份与安全通知服务端能力；G2A-04.1 的 `hosted.auth` / `hosted.account` 真实后端、G2A-05 管理 Blocks 与 G2A-06 用户 Blocks/Hosted UI 已 `verified`。当前唯一关口 G2A-07 为 `in_progress`：SDK、配置 Schema、Manifest 内容和 Generated Source 已通过本地脏工作区 Full 22/22，仍待提交与托管 CI。G2A-08 包内验证和 G2C 装配回归未完成前，不得晋级能力包。

管理后台与 Hosted UI/用户 Feature Block 已完成各自关口验证；Account 专用 SDK、能力配置和生成源码已有本地实现与专项证据，但尚未形成 clean commit 和托管 CI 证据。目标端装配、包内九面、升级/回滚和旧产品回归仍未完成。因此九个交付面尚未封口，该包不进入 ordinary 或 experimental runtime catalog，不能从创建入口选择，更不得标记为 `available`。

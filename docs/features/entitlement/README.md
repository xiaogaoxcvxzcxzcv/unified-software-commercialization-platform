...
当前唯一推进关口是 G2B-02：按 G2B-01 已封口模型实现 `000026_entitlement` 迁移、后端公开应用服务、PostgreSQL Adapter、API、幂等、Outbox 和范围/并发测试；不得提前进入 G2B-03 管理后台 Blocks。
...
下一步 G2B-02 的迁移起始编号必须是 `000026_entitlement`，因为当前仓库最后迁移为 `000025_hosted_self_service_flow`。

...
G1-07.1 已达到 `verified`：普通/实验 Generator/SDK 工具目录、Tool Manifest、内容树、执行入口、内置适配器白名单、兼容性与 Catalog Snapshot 已完成。G2C-01 已在受控实验工具根发布并本地验证 `platform.generator` 与 `platform.sdk` 的 verified tool manifest；普通工具根仍为空。
...
G1-08.3/G1-08.4、G1-10 和 G1-11 已达到 `verified`：创建/恢复、Lifecycle Plan/Operation、可信 Extension Catalog、确定性快照/计划和服务组合已实现；真实 PostgreSQL Full 18/18 及 GitHub push/PR 托管 `quality-gate` 均通过。当前唯一关口为 G2A-01。

普通生产能力包、UI 模板和受信 Generator/SDK 工具目录当前为空，因此普通创建向导按设计显示真实空状态并禁止继续，生产计划生成保持失败关闭。受控实验目录已有 `package.account` 1.0.0、`package.entitlement` 1.0.0、`standard-a` 0.1.0、`platform.generator` 1.0.0、`platform.sdk` 1.0.0 和样板 `extension.editor-tools` 1.0.0，并已通过 G2C-01 本地 Full 验证；experimental 路由必须由服务端显式授予 `assembly.experimental.use`。当前没有 `available` 完整能力包，单款软件管理工作区和业务页面仍未闭环。
...
G1-05 完成的是 Generator 与 Run 的后端执行闭包，不等于完整软件装配已经可交付。G1-06 至 G1-11 已完成 SDK/UI 基座、模板、工具/扩展目录基础、创建、工作区、durable Run、恢复、公开 lifecycle API 和可信扩展规划。Product Blueprint 至少需要一个真实能力包，因此第一次真实样板装配仍在 G2C。G2C-01 已证明 experimental account/entitlement/template/tool/extension 组合可解析为确定性计划；普通能力包、模板、扩展和真实工具版本仍为空，生产 Planner/Executor 继续失败关闭。

...
|---|---|---|---|---|---|---|---|---|---|
| auth.login | 登录 | P0 | page / modal | 可信上下文、登录方式、return_target | UserContext、login-completed、recoverable-error | 全端 | 微信扫码/授权、App OAuth、桌面浏览器回调 | identity | not_ready |
| auth.register | 注册 | P0 | page / modal | 可信上下文、注册策略、协议 | UserContext、registration-completed | Web/H5/桌面/App；小程序按策略 | 验证码、微信资料授权 | identity | not_ready |
| auth.recovery | 找回密码 | P0 | flow | 登录标识、验证方式 | recovery-completed、cooldown | 全端 | 短信/邮件/微信验证 | identity | not_ready |
| auth.second-factor | 二次验证 | P1 | modal / flow | 风险挑战、可用验证方式 | challenge-completed / cancelled | 全端 | 生物识别、OTP、微信确认 | identity | not_ready |
| account.center | 个人中心 | P0 | page / shell | UserContext、能力开关、摘要 | navigation event | 全端 | 端内导航结构 | identity | not_ready |
| account.profile | 个人资料 | P0 | page / panel | 用户摘要、允许编辑字段 | profile-updated | 全端 | 头像选择、微信头像昵称 | identity | not_ready |
| account.security | 账号安全与会话 | P1 | page / panel | 会话、绑定方式、安全事件 | password-changed、session-revoked | 全端 | 安全存储、生物识别 | identity | not_ready |
| entitlement.summary | 当前会员与权益 | P0 | card / page | 当前权益结论、功能清单 | renew-selected、upgrade-selected | 全端 | 无 | entitlement | not_ready |

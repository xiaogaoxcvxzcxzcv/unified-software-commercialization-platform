...
BEGIN
    IF EXISTS (
        SELECT 1 FROM hosted_interaction.interactions
        WHERE initiator_kind = 'user'
    ) THEN
        RAISE EXCEPTION 'migration 000022 rollback refused: Hosted user actor facts exist';
    END IF;
END;
$$;

ALTER TABLE hosted_interaction.interactions
    DROP CONSTRAINT IF EXISTS hosted_interactions_actor_session_shape,
    ALTER COLUMN initiator_client_session_id SET NOT NULL,
    ADD CONSTRAINT interactions_initiator_client_session_id_check
        CHECK (char_length(initiator_client_session_id) BETWEEN 1 AND 160);

...
export function TenantsPage() {
  ...
  return <Shell title="代理租户" subtitle={`${currentProduct?.name ?? "当前软件"} · 官方与代理经营范围隔离`}>
    ...
    {!query.loading && !query.error && <section className="panel table-panel"><div className="table-heading"><div><h2>租户列表</h2><p>官方租户由系统自动创建</p></div></div><div className="table-scroll"><table><thead><tr><th>租户</th><th>稳定代码</th><th>类型</th><th>上下文版本</th><th>状态</th></tr></thead><tbody>{query.data.map((item) => <tr key={item.id}><td><strong>{item.name}</strong></td><td><code>{item.code}</code></td><td><StatusBadge status={item.type}/></td><td>{item.contextVersion}</td><td><StatusBadge status={item.status}/></td></tr>)}</tbody></table></div></section>}

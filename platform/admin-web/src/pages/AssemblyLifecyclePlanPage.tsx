...
export function AssemblyLifecyclePlanPage() {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    setLoading(true); setError(null); setReauthenticationRequired(false); setIdempotencyConflict(false);
  ...
  return <Shell title="生命周期计划" subtitle={planId ?? "差异、冲突与回滚审查"}>
    ...
    {error && <div className="create-global-error" role="alert"><IconAlertTriangle size={18} /><span>{error}</span>{reauthenticationRequired && <button className="secondary-button" type="button" disabled={preparingLogin} onClick={() => void beginReauthentication()}>{preparingLogin ? "正在退出当前会话..." : "重新登录并返回计划"}</button>}{idempotencyConflict && <button className="secondary-button" type="button" onClick={restartExecuteIntent}>放弃旧意图并重新发起</button>}</div>}
